...
- **상세페이지 업로드 폴더 위치** (2026-05-29 결정 보류, 워크플로 토글 도입 시 함께 구현)
  - 동료가 GPT Image 2.0으로 만든 상세페이지 결과물(이미지)을 어디에 둘지.
  - **잠정 결정: 해당 제품 Drive 폴더의 하위 폴더(`{제품 폴더}/상세페이지/`)** — 원본 자료와 결과물 분리하면서 제품 단위 관리 유지.
  - 구현 시 함께 결정해야 할 사항:
    - 하위 폴더명 고정(`상세페이지`)으로 갈지 변형 허용할지
    - `pipeline/drive_client.py`에 `get_or_create_subfolder()` 헬퍼 추가 필요
    - `상품_파일` 테이블에 상세페이지 결과물을 어떻게 표시할지 (기존 `종류` 컬럼 활용 vs 신규 플래그)
  - 다른 후보(같은 폴더 / 별도 계정 전용 드라이브)는 추후 외주 공유 빈도가 높아지면 재검토.

#### 🆕 워크플로 토글 + 변경 감지 도입 (2026-05-29 협의 완료, 새 세션에서 구현 진입)

**배경 문제**
- 제품조회 페이지([pages/1_products.py](pages/1_products.py))의 4개 박스(완성/진행중/일반/테스트)가 "필드 충실도"(이미지+카테고리+제품특징_bullet 채워짐 여부) 기준이라 동료-나-동료 협업의 단계 표현 못 함
- 비전패스만 돌려도 필드가 채워져서 ✅완성 박스로 잡히는 위양성 발생 (실제 엠군 파이프라인은 안 돌았는데)
- 동료가 기초자료 입력을 "끝낸 상태"인지 "작업 중"인지 시스템이 구분 불가
- 후속 작업자가 silent edit(완료 토글 후 수정)을 놓침

**해결 방향**
- 박스 분류 기준을 "필드 충실도" → "워크플로 단계(어디까지 완료)"로 전환
- 단계 진행을 위한 **명시적 완료 토글 3개**
- 토글 ON 시점의 핵심 필드를 **JSON 스냅샷**으로 박제 → 후속 작업자가 페이지 열 때 변경 감지·알림

---

### A. DB 스키마 변경

**상품 테이블에 컬럼 6개 추가** (마이그레이션 `db/run_add_워크플로_토글.py` 신규)

| 컬럼명 | 타입 | 의미 |
|---|---|---|
| `기초입력_완료_at` | TIMESTAMPTZ NULL | 동료가 기초자료 입력 끝났음 토글 ON 시각 |
| `기초입력_완료_snapshot` | JSONB NULL | 토글 ON 시점의 기초입력 핵심 필드 박제 |
| `엠군_완료_at` | TIMESTAMPTZ NULL | 엠군 파이프라인 결과가 동료에게 넘기기 적합한 상태로 토글 ON 시각 |
| `엠군_완료_snapshot` | JSONB NULL | 토글 ON 시점의 엠군 결과(전체 run) 박제 |
| `상세페이지_완료_at` | TIMESTAMPTZ NULL | 동료가 상세페이지 생성 끝났음 토글 ON 시각 |
| `상세페이지_완료_snapshot` | JSONB NULL | 향후 채널 단계 작업자용 (현재는 빈 `{}` 유지) |

**기존 `엠군상태` enum과의 관계 — 병행 유지**
- 현재 `엠군상태`(미시작/진행중/완료)는 파이프라인 실행 시 자동 갱신되는 **기술적 상태**
- 새 `엠군_완료_at`은 사용자가 "이 결과를 동료에게 넘긴다"고 선언하는 **의도적 상태**
- 두 신호 의미가 다르므로 병행 유지 (자동 vs 수동 신호 분리)
- UI에서는 `엠군_완료_at` 중심으로 표시. `엠군상태`는 내부 디버그·자동 갱신용

**기존 데이터 마이그레이션 — 옵션 C 채택**
- 모든 기존 제품 → 모든 토글 NULL → 자동으로 ⬛ 일반 박스에 배치
- 동료가 진행 완료한 제품 2개만 수동으로 토글 찍어줌 (사용자가 동료에게 별도 안내)
- 자동 분류로 오분류될 위험 없음. 깨끗하게 새 출발.

---

### B. 박스 라이프사이클 재정의 (5개 박스)

| 박스 | 분류 조건 |
|---|---|
| ⬛ **일반** | 모든 토글 OFF (NULL) |
| 🟠 **기초자료 입력완료** | 기초입력 토글 ON, 엠군 토글 OFF |
| 🔵 **엠군 작업 완료** | 엠군 토글 ON, 상세페이지 토글 OFF |
| ✅ **상세페이지 생성 완료** | 상세페이지 토글 ON |
| 🧪 **테스트** | is_test=True (기존 그대로 유지, 논외) |

**"동료입력중" 박스는 만들지 않음**
- "입력 시작했지만 미완료" 상태 별도 박스 없음
- 동료가 본인 작업은 본인이 아니까 시스템이 추측할 필요 없음
- 토글 ON 전까지 모두 "일반"

**완성도 기준 폐기**
- 기존 `_completeness()` 함수([pages/1_products.py:51-58](pages/1_products.py:51)) 완전 제거
- "이미지 1장 이상 + 카테고리 + 제품특징_bullet" 기준 무의미
- 이미지/카테고리/제품특징 유무는 박스 분류와 무관 (단, 정보 컬럼으로는 표시 유지)

**"완성도" 컬럼 제거**
- [pages/1_products.py:139](pages/1_products.py:139) `df_all.insert(2, "완성도", df_all["_완성도"])` 라인 삭제
- 박스 자체가 곧 단계라 컬럼이 중복 (한 박스 내 모든 행이 같은 값이라 무의미)

---

### C. 변경 감지 (snapshot JSON, 옵션 B)

**원리**
- 토글 ON 시점에 "후속 작업자가 보고 일해야 하는 핵심 필드"를 JSON으로 박제
- 페이지 로드 시 현재 데이터와 비교 → diff 산출 → 🔄 알림

**단계별 snapshot 대상 필드**

🟠→🔵 **`기초입력_완료_snapshot`** (후속작업자=엠군 돌리는 사람)
- 카테고리
- 제품특징_bullet
- 가격
- 판매자_특성
- 파일_ids (image 타입만 — `📁파일` 컬럼이 image/video/etc 카운트하지만 엠군 input으로는 image만 의미 있음)
- 시각설명 (비전패스 결과 — 엠군 input)

🔵→🟣 **`엠군_완료_snapshot`** (후속작업자=상세페이지 만드는 동료)
- **엠군 결과 전체를 run 단위로 박제** (엠군은 타겟 단위 run으로 돌고, 한 제품에 타겟 여러 명 + 같은 타겟 결핍 다른 추가 run 가능)
- 박는 형태:
  ```json
  {"runs": [
    {"target":"A","run_id":1,"결핍":"...","포지셔닝":"...","04_a_writing":"...","04_1_image_direction":"..."},
    {"target":"A","run_id":2,"결핍":"(다른 결핍)","포지셔닝":"...", ...},
    {"target":"B","run_id":3, ...}
  ]}
  ```
- 새 run 추가, 기존 run 수정, run 삭제 → 어떤 형태든 변경 감지 가능
- run 내부 디테일 비교까지 갈지: 핵심 결과 해시 정도면 충분 (너무 깊으면 false positive)

🟣→✅ **`상세페이지_완료_snapshot`**
- 현재 후속 작업자 없음 → 일단 빈 `{}` 유지
- 향후 05 채널 단계 작업자 도입 시 필드 채움
- 컬럼은 미리 만들어둠 (마이그레이션 두 번 안 하게)

**snapshot 필드 관리 — 설정 페이지 UI 신설**
- [pages/0_settings.py](pages/0_settings.py)에 "📸 변경 감지 필드 관리" 섹션 신설
- 기존 "참고필드 관리"와 같은 패턴 — 단계별 멀티셀렉트
- 저장: `settings.json`의 `DEFAULTS`에 `snapshot_fields_기초입력`, `snapshot_fields_엠군`, `snapshot_fields_상세페이지` 키 추가
- 사용자가 자유롭게 체크 풀고/켜기 가능
- **비교 로직이 변동에 강건하므로 안전:**
  - "snapshot에 없는 키는 비교 대상에서 제외" → 필드 추가 시 false positive 없음
  - "snapshot에 있는데 현재 없는 키도 무시" → 필드 삭제 시 안전
- **참고필드 관리와의 관계**: 두 가지 의미가 다름
  - 참고필드 관리 = 엠군 파이프라인이 input으로 받는 필드
  - snapshot 필드 = 후속 작업자에게 변경을 알릴 필드
  - 겹치는 영역: 참고필드 관리에서 제외된 필드 = snapshot에서도 제외해도 됨 (단일 소스로 자동 차감 처리 가능, 선택 사항)

---

### D. 확인 메커니즘 — 단일 [✓ 확인했음] 버튼

**원리**
- 변경 감지된 상태에서 후속 작업자가 [✓ 확인했음] 클릭
- 동작: 본인 단계 토글의 `at` + `snapshot` 갱신 (재찍기)
- 효과: diff 사라짐 → 🔄 뱃지 사라짐

**의미 구분은 사람 머릿속에서**
- **반영함**: 내 단계 작업물(엠군 결과 / 상세페이지)을 새로 만들고 → 확인했음
- **무시함**: 내 단계 작업물 그대로 두고 → 확인했음
- 시스템은 둘 구분 안 함 (의도 로그가 진짜 필요해지면 향후 버튼 2개로 분리 가능)

**UI 위치**: 변경 감지 카드 안에 버튼 1개

---

### E. UI 변경 (Streamlit)

**E-1. 토글 UI 위치 — 옵션 A 확정**
- [pages/2_product_edit.py](pages/2_product_edit.py)의 페이지 상단 헤더
- 기존 "엠군 대상 (ON/OFF)", "테스트 레코드 (ON/OFF)" 토글 영역([pages/2_product_edit.py:218-237](pages/2_product_edit.py:218)) 옆/아래에 워크플로 토글 3개 추가
- 기존 토글과 동일한 패턴 사용 — **`_clear_lock_after_self_edit()` 호출 필수** (편집 세션 락 시스템)
- `_btn_col4/5/6` 으로 columns 확장하거나, 한 줄 아래에 새 columns
- 일괄 토글(옵션 B, 박스 안에서 여러 제품 한번에)은 **보류** — 운영해보고 필요해지면 추가

**E-2. 박스 헤더 — 단계명만, 인칭 표현 없음**
- '나/동료' 같은 인칭 사용 금지 (시점 의존적이라 헷갈림)
- 표시 예시:
  ```
  🟠 기초자료 입력완료 · 5건  ⚠️ 1건 수정됨
  🔵 엠군 작업 완료 · 3건  ⚠️ 2건 수정됨
  ✅ 상세페이지 생성 완료 · 12건
  ```
- 박스명 자체가 "이전 단계까지 완료"를 의미. 다음 단계는 옆 박스라 자명 (4개 박스가 순서대로 배치).

**E-3. 페이지 상단 알림 배너**
- 페이지 진입 시 한눈에 "다음 단계 대기" 파악
- 표시 예시:
  ```
  🔔 엠군 파이프라인 실행 대기: 5건  ·  상세페이지 제작 대기: 3건
  ```
- 단계 액션명으로 표현, 인칭 표현 없음

**E-4. 테이블 시각화**
- (a) 테이블 첫 컬럼에 `🔄 수정됨` 컬럼 추가
- (b) `df.sort_values(by="🔄", ascending=False)` — 변경된 행 자동 상단 정렬
- (c) 행 클릭 시 상세 패널에 diff 리스트 + [✓ 확인했음] 버튼
- Pandas Styler 셀 색칠은 비추 — 한국어 폰트/이모지와 정렬 깨짐

**E-5. 행별 owner 컬럼은 추가 안 함**
- 박스가 이미 owner별로 분리되어 있어 중복

---

### F. 코드 변경 영향 분석 (2026-05-29 시점 코드 기준)

작업 진입 전 알아둘 최근 변경사항 (다른 채팅방에서 진행됨, 주로 이미지 갤러리·Drive 관련):

1. **`📁파일` 컬럼** ([pages/1_products.py:132-138](pages/1_products.py:132))
   - 이전 `🖼️이미지` (단순 개수)에서 image/video/etc 카운트로 변경
   - 관련 함수: [pipeline/supabase_read.py:458, 482](pipeline/supabase_read.py:458) — `get_file_counts_by_type()`, `format_file_counts()`
   - snapshot에 박을 때는 `image` 타입만 추리는 게 의미상 정확 (엠군 input으로는 비디오/기타 파일 무관)

2. **`pipeline.role` 모듈 신설** — `current_username`, `is_owner` 함수
   - 사용자 역할 시스템 도입됨
   - 워크플로 토글에 역할별 권한(누가 어떤 토글 찍을 수 있나) 거는 것 향후 고려 대상
   - 일단은 권한 안 검 — 모든 토글 자유롭게. 운영해보고 필요하면 추가

3. **편집 세션 락 시스템** — `get_active_편집_세션`, `_clear_lock_after_self_edit()` 패턴
   - 누가 편집 중인지 트래킹
   - **워크플로 토글 추가 시 같은 패턴으로 `_clear_lock_after_self_edit()` 호출 필수**
   - 기존 토글 코드([pages/2_product_edit.py:218-237](pages/2_product_edit.py:218)) 그대로 패턴 따라가면 됨:
     ```python
     if st.button(...):
         set_워크플로_토글(product_id, "기초입력", not 현재값)
         _clear_lock_after_self_edit()
         st.rerun()
     ```

4. **`pages/5_image_match.py` 신규** — 이미지-상품 매칭 페이지 추가됨
   - 본 작업에 직접 영향 없음 (정보용)

5. **신규 import들** ([pages/2_product_edit.py:33-65](pages/2_product_edit.py:33))
   - `pipeline.account_ui`, `pipeline.image_actions`, `pipeline.role` 등 새 모듈
   - 본 작업에서 새 모듈 신설은 없음 — `pipeline/supabase_read.py`에 헬퍼만 추가

---

### G. 구현 진입 순서 (권장)

1. **마이그레이션 스크립트** — `db/run_add_워크플로_토글.py` 신규
   - 패턴 참고: `db/run_add_제품특징_영역.py`, `db/run_add_박스_및_수입자_사용연령.py`
   - 멱등하게 (`ADD COLUMN IF NOT EXISTS`)
   - 컬럼 6개 추가 (위 A 섹션 표 참조)
   - **Claude가 직접 실행** (사용자 메모 `feedback_db_migration_auto` — 멱등·안전한 마이그레이션은 Claude 직접 실행, 사용자에게 떠넘기지 말 것)

2. **`pipeline/supabase_read.py`** — 헬퍼 함수 추가
   - `set_워크플로_토글(product_id: int, stage: str, on: bool)` — `at` 갱신 + `snapshot` 박제 (ON 시) / 둘 다 NULL (OFF 시)
   - `compute_워크플로_단계(row: dict) -> str` — 토글 조합 → 박스 분류 ("일반"/"기초입력완료"/"엠군완료"/"상세완료")
   - `compute_변경_diff(row: dict, stage: str) -> list[dict]` — snapshot vs 현재 비교 → diff 리스트 (필드명, 이전값, 현재값)
   - `acknowledge_변경(product_id: int, stage: str)` — 확인했음 (재토글, snapshot 갱신 only, at는 그대로 둘지 갱신할지는 구현 시 결정 — 일단 둘 다 갱신 권장)
   - 엠군 단계 snapshot 생성 헬퍼 — 한 제품의 모든 엠군 run 결과 끌어서 `{"runs": [...]}` 형태로 직렬화

3. **`pipeline/settings.py`** — `DEFAULTS`에 `snapshot_fields_기초입력`, `snapshot_fields_엠군`, `snapshot_fields_상세페이지` 키 추가 + load/save 함수

4. **`pages/0_settings.py`** — "📸 변경 감지 필드 관리" 섹션 신설 (단계별 멀티셀렉트)

5. **`pages/1_products.py`** — 큰 수정
   - `_completeness()` 함수 제거, `_워크플로_단계()` 신규 함수로 대체
   - 박스 정의 4개로 재작성 (`_COMP_*` 상수 → `_STAGE_*` 상수, 테스트는 유지)
   - "완성도" 컬럼 제거 ([1_products.py:139](pages/1_products.py:139))
   - 🔄 컬럼 추가 + 자동 정렬 (변경된 행 위로)
   - 박스 헤더에 변경 카운트 (`⚠️ N건 수정됨`)
   - 페이지 상단 알림 배너 (`🔔 엠군 파이프라인 실행 대기: N건 · 상세페이지 제작 대기: M건`)

6. **`pages/2_product_edit.py`** — 헤더 토글 + 변경 감지 카드
   - 기존 토글 영역([2_product_edit.py:218-237](pages/2_product_edit.py:218)) 옆/아래에 워크플로 토글 3개 추가
   - 변경 감지 카드 (diff 리스트 + [✓ 확인했음] 버튼) — 토글 영역 바로 아래 또는 탭 위 공통 영역
   - `_clear_lock_after_self_edit()` 호출 패턴 준수

---

### H. 관련 보류 항목

- **상세페이지 업로드 폴더 위치** — 위 ⏸️ 보류 섹션 참고 (잠정: `{제품}/상세페이지/` 하위 폴더). `상세페이지_완료_at` 토글 동작과 연계되므로 이 워크플로 토글 구현 시 함께 결정·구현 가능.

---

### I. 결정 사항 한눈 정리 (2026-05-29 협의 완료)

| # | 항목 | 결정 |
|---|---|---|
| 1 | 박스 라이프사이클 | ⬛일반 / 🟠기초입력완료 / 🔵엠군완료 / ✅상세완료 + 🧪테스트 (5개) |
| 2 | "동료입력중" 박스 | 만들지 않음 |
| 3 | 완성도 기준·컬럼 | 폐기 (이미지/카테고리/제품특징 무관) |
| 4 | 워크플로 토글 | 3개 (기초입력_완료_at / 엠군_완료_at / 상세페이지_완료_at) |
| 5 | 기존 데이터 마이그레이션 | 옵션 C — 전부 NULL (자동 ⬛ 일반) |
| 6 | 변경 감지 방식 | 옵션 B — snapshot JSON |
| 7 | snapshot 필드 단위 | 단계별 별개 목록 |
| 8 | snapshot 필드 관리 UI | 설정 페이지에 "📸 변경 감지 필드 관리" 섹션 신설 |
| 9 | 엠군 단계 snapshot | run 단위로 박제 (모든 run 리스트) |
| 10 | 상세페이지 이후 snapshot | 컬럼은 만들고 빈 `{}` 유지 |
| 11 | 확인 메커니즘 | 단일 [✓ 확인했음] 버튼 → 재토글 |
| 12 | 토글 UI 위치 | 옵션 A — `2_product_edit.py` 상단 헤더 |
| 13 | 일괄 토글 (옵션 B) | 보류 |
| 14 | 박스 헤더·상단 배너 표현 | 단계명 그대로 (인칭 X) |
| 15 | 테이블 시각화 | 🔄 컬럼 + 자동 정렬 + 상세 패널 diff |
| 16 | 행별 owner 컬럼 | 추가 안 함 (박스가 이미 분리) |
| 17 | `엠군상태` enum과의 관계 | 병행 유지 (자동 vs 수동 신호 분리) |

**→ 추가 결정 필요 없음. 새 세션에서 G 섹션의 구현 진입 순서대로 진행.**
...
## 🔖 다음 채팅방 시작 시 안내 (2026-05-25 시점, Phase 0 완료)
...
## 🔖 다음 채팅방 시작 시 안내 (2026-05-27 시점, Phase 1~4 완료) — 최신

### 직전 세션에서 한 일 (2026-05-25 ~ 2026-05-27 — 동시편집 보호 · Cloud OAuth fallback · Drive 대시보드)

결정 로그 [`2026-05-25_streamlit_cloud_배포.md`](docs/decisions/2026-05-25_streamlit_cloud_배포.md) §3 Phase 1~5 전체 [x] 처리 완료.

#### git 이력 (자동화형 repo, **push 안 됨** — GitHub master는 `351a514` 그대로)

| commit | 내용 |
|---|---|
| `30655bc` | Phase 1 — 편집_세션 · drive_auth · 상품_수정일_갱신 트리거 마이그레이션 + supabase_read 함수 5개 |
| `6a3c693` | Phase 2 — runtime.is_streamlit_cloud / drive_client DB fallback / 2_product_edit 동시편집 + 낙관적 락 |
| `ea64391` | Phase 3·4 — Drive 대시보드 + ACCOUNTS 역할 + quota 함수 + secrets.toml.example |

#### 새로 들어온 인프라

- **DB**: `편집_세션`, `drive_auth` 테이블 + `상품_수정일_갱신` BEFORE UPDATE 트리거 (결정 로그 §2-6 부록 — 트리거 추가 사유)
- **pipeline/runtime.py** 신규: `is_streamlit_cloud()` 헬퍼
- **pipeline/role.py**: `current_username()` 추가 (owner(나) / partner(동료))
- **pipeline/drive_client.py**:
  - `ACCOUNTS`에 `"역할"` 필드 (voyager=메인 운영, donnamoo=보조 운영)
  - `_get_credentials`: pickle 우선 + `drive_auth` DB fallback + refresh 시 양쪽 동기화
  - `get_account_quota(name)`: `about(storageQuota,user)` 기반 quota·email
- **pipeline/supabase_read.py**:
  - `update_상품(original_수정일=None)` — 낙관적 락 지원
  - `get_drive_token` / `upsert_drive_token`
  - `upsert_편집_세션` / `get_active_편집_세션(ttl_min=5)`
  - `count_파일_by_계정`
- **scripts/refresh_oauth_token.py**: 발급 후 `upsert_drive_token` 자동 호출
- **pages/2_product_edit.py**: 진입 시 편집 세션 등록(30초 debounce, 임시 레코드 제외) + 다른 사용자 편집 알림 배지 + 저장 3곳에 낙관적 락
- **pages/5_drive_dashboard.py** 신규: 계정 카드(역할·이메일·상태·잔여용량 progress·파일수·토큰동기화시각) + 추천 다음 업로드 계정 헤더 + OAuth 재인증 가이드 expander
- **.streamlit/secrets.toml.example**: Cloud Secrets 템플릿

#### 검증 결과

- `상품_수정일_갱신` 트리거 BEFORE UPDATE 작동 (수정일 자동 갱신)
- 낙관적 락: 잘못된 `original_수정일` → `False` 반환 (충돌 감지)
- 두 사용자 시뮬레이션: owner/partner UPSERT 후 상호 알림 정상 + UNIQUE(상품_id, 사용자명)로 중복 row 방지
- voyager / donnamoo 토큰 — 첫 `build_service` 호출에서 `drive_auth` 자동 동기화 완료 (155MB/15GB · 4MB/15GB · 이메일 정확)

#### 결정 변경 (이번 세션에서 굳어진 것)

- **§2-6 A 부록**: 결정 로그 작성 시 "수정일 추가 마이그레이션 불필요" 가정 → 실제로는 UPDATE 트리거 부재. `상품_수정일_갱신` 트리거 추가로 보강.
- **§3 Phase 3-3**: 역할 라벨은 DB 컬럼이 아닌 **코드 `drive_client.ACCOUNTS` 단일 소스** 채택. 마이그레이션 안 함.
- **OAuth 재인증 1차 구현**: 가이드 expander만 (로컬 `refresh_oauth_token.py` 실행 안내). 실제 OAuth callback 통합은 Backlog.
