...
### 🟢 [자동화형 재작업 시] 04_b RETURN 자동 처리 정책 — 동일 모델 루프 주의

> ⚠️ **지금 도입 X.** 대화형 04_b가 성숙해 자동화형을 전면 재작업할 때 같이 결정할 사항. 현행은 안전(아래 "현재 상태" 참조)하니 손대지 말 것.

- **현재 상태 (루프 없음)**: `pipeline/auto_pipeline.py:run_stage_04_b`는 검수 보고서 + 다듬은 콘티를 만들어 DB에 저장만 함. 04_a를 자동 재호출하지 않으므로 RETURN은 **보고서 안의 분류 텍스트**일 뿐, 자동 루프가 없다. 즉 "무한 핑퐁"은 현재 발생하지 않음.
- **염려사항 (2026-05-30 04_b Tier 분리 작업 중 발견)**: 향후 자동 RETURN(04_a 자동 재호출)을 붙이면, RETURN은 04_a(=**같은 모델**)에게 돌려보내는 것이라 문제가 생긴다. 방금 박은 **동일 모델 검수 원칙**(`_공통 두뇌/04_b_review/core.md` §판단원칙 0)상, **Tier 2 판단 플래그를 자동 RETURN으로 돌리면 같은 모델이 똑같이 다시 뱉을 공산**이 커서 횟수만 태우고 안 고쳐지는 무한 루프 위험.
- **권장 방향 (재작업 시 검토)**:
  - Tier 1(기계적 미스: ENGINE_PLAN 누락·yaml 키 누락 등)만 **자동 RETURN 1회** 허용 → 또 실패면 보고서 플래그 달고 통과.
  - Tier 2(풍부도·한 축·시각비중·강사카드·핵심반론 판단)는 **자동 RETURN 금지** → 곧장 사람(또는 보고서) 플래그. 횟수 가드보다 "Tier 2는 애초에 자동 RETURN 대상이 아니다"가 핵심.
- **선례**: `docs/decisions/2026-05-02_pipeline_quality_pass.md` — 02→04→05 cascade도 "무한 재실행" 우려로 자동 재실행을 거부하고 ⚠️배지+🔄수동버튼으로 감. 같은 철학(수동 우선) 적용 가능.
- **관련 파일**: `agents/04_b_review/instruction.md`(Tier 1/2 분리·DIRECT/RETURN 매핑), `_공통 두뇌/04_b_review/core.md` §판단원칙 0, `pipeline/auto_pipeline.py:run_stage_04_b`.
