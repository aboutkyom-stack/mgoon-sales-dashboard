...
## 🔖 다음 채팅방 시작 시 안내 (2026-05-25 시점) — 최신
...
#### ⭐ 본 세션에서 확정된 운영·배포 결정사항

**[docs/decisions/2026-05-25_streamlit_cloud_배포.md](docs/decisions/2026-05-25_streamlit_cloud_배포.md) ⭐ 다음 세션 시작 시 정독 필수**

요약:
- **배포**: Streamlit Cloud 무료 + Public GitHub repo + Restricted access (사용자+동료 이메일만)
- **동료에게 줄 것**: URL만 (코드/Python/.env 어떤 것도 동료 PC에 없음)
- **Drive OAuth**: 본인 계정으로 신규 발급 + 동료 계정은 점진적 전환 (안정화 후 제거)
- **Drive 토큰 동기화**: Supabase DB에 refresh_token 저장 → 로컬 스크립트가 갱신 시 자동 update → Cloud는 DB read만
- **동시편집 보호**: 옵션 A(낙관적 락, `상품.수정일` 활용) + 옵션 B(`편집_세션` 테이블)
- **push 자동화**: 명시적 요청만 (Streamlit Cloud는 push 즉시 재배포되므로 자동화 위험)
- **Drive 계정 대시보드 신규 요청**: 모든 연동 계정의 용량·역할·잔여를 한눈에 보는 페이지 (Phase 3)

### 새 채팅방 시작 시 가장 먼저

1. 이 `PROJECT_STATUS.md` (현재 섹션) 읽기
2. **[docs/decisions/2026-05-25_streamlit_cloud_배포.md](docs/decisions/2026-05-25_streamlit_cloud_배포.md)** 정독 — 전체 결정사항 + Phase 1~5 작업 체크리스트 + 사전 확인 사항
3. §3 체크리스트를 TaskCreate로 옮긴 후 Phase 1부터 순차 진행

### 다음 우선순위 (결정 로그 §3 참조)

- **Phase 1**: DB 스키마 & 백엔드 (편집_세션 테이블, drive_auth 컬럼/테이블, supabase_read 함수들, 낙관적 락 지원)
- **Phase 2**: 코드 분기 로직 (drive_client DB fallback, refresh_oauth_token DB 자동 update, 동시편집 통합)
- **Phase 3**: Drive 계정 대시보드 (quota 조회 + 신규 페이지 + 역할 라벨)
- **Phase 4**: 배포 준비 (환경 감지, secrets 템플릿)
- **Phase 5**: GitHub + Streamlit Cloud 셋업 (사용자 직접 + Claude 보조)

### 핵심 사전 확인 사항

- 기존 `drive_accounts` 테이블 스키마 (컬럼 추가 vs 신규 테이블 분리 결정)
- 현재 git remote 상태 (아직 GitHub 연결 안 됨)
- OAuth client 본인 명의 발급 여부 (사용자 직접 작업)
- Streamlit Cloud 가입 여부 (사용자 직접 작업)
