...
SQL_FILE = Path(__file__).parent / "add_is_test.sql"
...
with psycopg2.connect(DB_URL) as conn:
    with conn.cursor() as cur:
        cur.execute("""
            ...
            WHERE table_name = '상품' AND column_name = 'is_test';
        """)
        rows = cur.fetchall()

if not rows:
    print("[FAIL] is_test 컬럼 생성 확인 실패")
    sys.exit(1)

print("\n[OK] 상품.is_test 컬럼 추가 완료")
for col, dtype, default, nullable in rows:
    print(f"  - {col}: {dtype}, default={default}, nullable={nullable}")
